...
5 REM == CPU f (MHz)	time (s, Fast Ext Bas)	==
6 REM == 6.25		115.892			==
7 REM == 12.5		58.720			==
8 REM == 25.0		30.135			==
9 REM == 100.0		8.435			==
...
170    	Y=R+R+D, X=T, I=I+1 : GOTO 110
210    	O=USR(3, I, 7)	:REM O=I&7 END BOTH WHILE... Loops
...
1001 S=1: if G<0 then let S=-1
1002 if N<0 then let S=-S
1010 L=ABS(G), Z=ABS(N)
1031 push x, y: x=usr(3, L, 255), y=usr(3, Z, 255), R=x*y
1041 let R=((L/256)*(Z/256)*256)+((L/256)*y)+(x*(Z/256))+(R/256)
1050 let R=S*R : pop y, x: RETURN
